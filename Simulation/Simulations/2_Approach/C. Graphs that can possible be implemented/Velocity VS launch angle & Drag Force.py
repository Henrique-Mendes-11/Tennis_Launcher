...
##### Graph - Velocity VS launch angle with drag force

g1 = vp.graph(title='Range VS launch angle', xtitle='theta [º]', ytitle='range [m]', width = 500, height = 250)
g2 = vp.graph(title='Velocity VS launch angle', xtitle='v0 [m/s]', ytitle='max theta [º]', width = 500, height = 250)
f1 = vp.gcurve(color=vp.color.blue)
f2 = vp.gcurve(color=vp.color.orange)
...
def prange(ttheta, v0, r0):
    ...
    # Drag force
    radius = ball_radius_cm/100 #m
    rho = 1.2 #kg/m^3 (density of air)
    A = mt.pi*radius**2 #m^2
    C = ball_sphere_drag_coefficient
    
    while r.y >= 0:
        F = m*g - 0.5*rho*C*A*vp.mag(p/m)**2*vp.norm(p)
        p = p + F*dt
        r = r + p*dt/m
        t = t + dt
    return (r.x)
...
while v0 < 15:
    ...
    while theta < mt.radians(90):
        ...
        theta = theta + dtheta
    f2.plot(v0, find_max(range_theta_matrix)[1])
    v0 = v0 + dv
...
# EXPLANATION
# F2 means: If i launch the ball with a higher velocity, the angle that will give me the max range decreases
# If i throw balls with different sizes the max range differ, beacuse the drag force is different, volume is different