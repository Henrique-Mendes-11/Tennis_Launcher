...
# Ball 
ball.m = ball_mass_kg
ball.v = v0*vp.vector(mt.cos(theta),mt.sin(theta),-mt.cos(phi)) 
ball.p = ball.m*v0*vp.vector(mt.cos(theta),mt.sin(theta),-mt.cos(phi)) 
...
while ball.pos.y >= ground.pos.y+ball.radius+ground.size.y/2:
    ...
    # Force on the ball 
    F = ball.m*g - 0.5*rho*C*A*vp.mag(ball.p)**2*vp.norm(ball.p)/ball.m**2 
    ...
    varrow1.pos = ball.pos 
    varrow1.axis = vscale1*ball.v 
