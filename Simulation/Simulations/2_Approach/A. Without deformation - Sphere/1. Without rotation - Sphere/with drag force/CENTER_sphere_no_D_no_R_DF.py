...
ball = vp.sphere(pos=pos_init, radius=ball_radius_m, color=vp.color.orange, make_trail=True) 
# In "right":
#ball = vp.sphere(pos=vp.vector(-round(court_length/2, 3),avg_dist_cp,singles_court_width/2), radius=0.08, color=vp.color.yellow, make_trail=True)
...
varrow1 = vp.arrow(pos=ball.pos, axis=vscale1*ball.v, color=vp.color.cyan)
...
while ball.pos.y >= ground.pos.y+ball.radius+ground.size.y/2: #While the ball is above the ground
    ...
    # Force on the ball 
    F = ball.m*g - 0.5*rho*C*A*vp.mag(ball.p)**2*vp.norm(ball.p)/ball.m**2 #Drag force
    ...
    varrow1.pos = ball.pos #vector location of the velocity arrow
    varrow1.axis = vscale1*ball.v #vector velocity of the ball
...
#print("Ball 1 | Time to hit the ground: ", t_final1, "s")    
