# Rotation of the Ellipsoid test DRAG FORCE 
import json
import math as mt
import numpy as np
import pandas as pd
...
# Tennis variables: Ellipsoid - Oblate spheroid at the beggining and then prolate spheroid the rest of the time
...
ellipsoid = vp.ellipsoid(pos=vp.vector(x0,avg_dist_cp,0), length=ellipsoid_a_m, height=ellipsoid_c_m, width=ellipsoid_b_m, texture=vp.textures.earth, make_trail=False)
# In "right":
#ball = vp.sphere(pos=vp.vector(-round(court_length/2, 3),avg_dist_cp,singles_court_width/2), radius=0.08, color=vp.color.yellow, make_trail=True)
...
while ellipsoid.pos.y >= ground.pos.y+ellipsoid.radius+ground.size.y/2: 
    vp.rate(1000) #put at 10 to see the rotation of the ball | put at 1000 to see the trajectory of the ball along with 'make_trail=True' in the Ball
    
    # Force on the ellipsoid
    F = ellipsoid.m*g - 0.5*rho*C_ellipsoid*A_ellipsoid*vp.mag(ellipsoid.p)**2*vp.norm(ellipsoid.p)/ellipsoid.m**2 #Drag force
