...
ellipsoid = vp.ellipsoid(pos=pos_init, length=ellipsoid_a_m, height=ellipsoid_c_m, width=ellipsoid_b_m, color=vp.color.yellow, make_trail=True)
# In "right":
#ball = vp.sphere(pos=vp.vector(-round(court_length/2, 3),avg_dist_cp,singles_court_width/2), radius=0.08, color=vp.color.yellow, make_trail=True)
...
# CENTER
# When impacted the ball turns into an prolate spheroid and at the trejaectory turns into oblate spheroid 
...
while ellipsoid.pos.y >= ground.pos.y+ellipsoid.radius+ground.size.y/2: 
    ...
    #reference_frame.axis = ellipsoid.v
...
#print("Ball 1 | Time to hit the ground: ", t, "s")  
